...
def signal_process( Qin, Qdata, pulse_a, Nseg, Nplot, fs, maxdist, temperature, functions, stop_flag  ):
    ...
    while(  not stop_flag.is_set() ):
        ...
        #print(chunk.shape, pulse_a.shape)
...
def rtsonar( f0, f1, fs, Npulse, Nseg, Nrep, Nplot, maxdist, temperature, functions ):
    ...
    # initialize plot
    fig = bk.figure(height = 400, width = 800, title = 'Sonar',  y_axis_label = "Time [s]", x_axis_label = "Distance [cm]",
    ...
    t_rec = threading.Thread(target = record_audio,   args = (Qin, p, fs, stop_flag, None, Npulse  ))
